...
The models are very simple and only use the past bloom dates observed at each locations---no other covariates or information are considered.
...
tibble(location = 'vancouver',
       year = 2022) %>% 
  bind_cols(predicted_doy = round(predict(ls_fit_for_van, newdata = .))) %>%
  left_join(read_csv("data/vancouver.csv")) %>%
  select(location, year, bloom_doy, predicted_doy)
```

If satisfied with the predictions, we can now them to the predictions for the other sites:
```{r}
